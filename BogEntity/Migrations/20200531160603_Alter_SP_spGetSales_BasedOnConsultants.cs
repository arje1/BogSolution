...
{
    public partial class Alter_SP_spGetSales_BasedOnConsultants : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            string SqlScript = 
        @"ALTER PROC [Analytics].[spGetSales_BasedOnConsultants]
        (
        @StartDate DATE,
        @EndDate DATE,
		@PageNumber INT,
		...
		SELECT COUNT(s.Id) FROM [Sales].[Sale] s
        ...
        c.PrivateNumber AS ConsultantPrivateNumber, SUM(ps.ProductUnit) AS ProductUnitSum, SUM(ps.ProductUnit * ps.PricePerUnit) AS ProductPriceSum, @TotalNumber AS TotalNumber
        FROM [Sales].[Sale] s
